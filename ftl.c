...
//	char blank[SECTOR_SIZE];
	...
	int b_size = SECTOR_SIZE - strlen(sectorbuf);
	blank = (char *)malloc(sizeof(char)*(b_size));
	//blank[b_size - 2] = '#';
	//blank[0] = '\0';
	
	strncpy(blank, temp, b_size);


	//printf("%s::%d", temp,sizeof(temp));
	...
	if (sectormaptbl.entry[lsn].ppn != -1) // '뭔가 값이 있었다면'
	{
		...
		;
	printf("g:test %d\n",garbagetable.list[0].on_garbage);
